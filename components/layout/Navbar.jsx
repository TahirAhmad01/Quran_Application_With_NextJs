  ...
  return (
    <div className="px-5 bg-white/60 dark:bg-slate-900/60 text-gray-900 dark:text-gray-150 backdrop-blur-md transition-all duration-300 fixed top-0 left-0 right-0 w-full z-50 border-b border-white/20 dark:border-slate-800/60 shadow-sm">
      <div className="max-w-screen-2xl mx-auto py-3.5 flex justify-between items-center">
        {/* Logo */}
        <div className="text-2xl font-bold bg-gradient-to-r from-primaryColor to-emerald-650 dark:from-primaryColor-light dark:to-emerald-400 bg-clip-text text-transparent">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`text-sm font-semibold transition-all duration-200 relative py-1 hover:text-primaryColor ${
                  isActive 
        ...
        {/* Utility Buttons (Settings & Mobile Menu) */}
        <div className="flex items-center gap-2">
          <button
            onClick={openSettings}
            aria-label="Open settings"
            className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-700 dark:text-gray-300 hover:text-primaryColor dark:hover:text-primaryColor transition-all duration-200"
          >
            <IoSettings size={20} />
          </button>
          ...
          <button
            ...
            className="md:hidden p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-700 dark:text-gray-300 transition-all duration-200"
          >
