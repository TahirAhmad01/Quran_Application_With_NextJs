      ...
      {data?.map((surah, idx) => {
        return (
          <Link href={`/surah/${surah?.number}`} key={idx}>
            <div className="w-full bg-white dark:bg-gray-800 p-5 rounded-md shadow-sm flex items-center border border-transparent dark:border-gray-700 hover:border hover:border-red-500 group max-h-full overflow-hidden transition-colors my-1">
              <div className="h-12 w-16 bg-gray-200 dark:bg-gray-700 group-hover:text-white group-hover:bg-red-500 rotate-[496deg] text-black dark:text-gray-100 flex items-center justify-center rounded-md transition-colors">
